...
test_serializeStdDesig = actual @?= expected
  where
    actual   = serialize StdDesig { desigEntSing = Just "Taro"
                                  ...
                                  , desigId      = 50
                                  , desigIds     = [50..55] }
    expected = quoteWith std . T.intercalate d $ [ "Taro", "mhuman", "Don'tCap", "50", "[50,51,52,53,54,55]" ]
...
test_deserializeStdDesig = actual @?= expected
  where
    actual   = deserialize . quoteWith std . T.intercalate d $ [ "", "fhuman", "DoCap", "55", "[55,54,53,52,51,50]" ]
    expected = StdDesig Nothing "fhuman" DoCap 55 [ 55, 54..50 ]
