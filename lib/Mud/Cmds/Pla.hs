...
{-# LANGUAGE DuplicateRecordFields, FlexibleContexts, LambdaCase, MonadComprehensions, MultiWayIf, NamedFieldPuns, OverloadedStrings, ParallelListComp, RecordWildCards, TupleSections, TypeApplications, ViewPatterns #-}
  ...
  where
    helper ms = let p            = getPla i ms
                    ...
                    combineds = [ padName abbrev <> suffix
                                ...
                                | abbrev      <- styleAbbrevs Don'tCoins Don'tQuote . map fst $ singSuffixes' ]
...
mkBar x txt (c, m) = let ratio  = c `divide` m
                         ...
                         reds   = x - greens
                         a      = colorWith greenBarColor . T.replicate greens $ " "
...
chan (NoArgs i mq cols) = getState >>= \ms ->
    ...
    in do logPlaExecArgs "chan" [] i
          multiWrapSend mq cols . helper (styleAbbrevs Don'tCoins Don'tQuote chanNames) $ chanTunings
        ...
        found match =
            ...
            in mapM (updateRndmName i . view _1) nonLinkeds >>= \rndmNames ->
                let combo       = linkeds ++ zipWith (\rndmName -> _2 .~ rndmName) rndmNames nonLinkeds
                    ...
                    styleds     = styleAbbrevs Don'tCoins Don'tQuote . select _2 $ ins
          ...
          then wrapSend mq cols . sorryIncog $ "connect"
            ...
            (res,        Just ci)
              ...
              , toSelf    <- (focusingInnateMsg <>) $ case targetSings of
                ...
                _   -> T.concat [ "you connect the following people to the ", dblQuote cn, " channel: ", commas targetSings, "." ]
              ...
              disconnectHelper i (mkLastArgWithNubbedOthers as) idNamesTbl |&| modifyState >=> \case
                ...
                (res,        Just ci)
                  ...
                  , toSelf    <- (focusingInnateMsg <>) $ case targetNames of
                    ...
                    _   -> T.concat [ "you disconnect the following people from the ", dblQuote cn, " channel: "
                                    , commas . map format $ targetNames, "." ]
                  -> do toOthers <- mkToOthers ms otherIds targetIds cn
  ...
  where
    helper ms = let { originId = getRmId i ms; originRm = getRm originId ms } in case findExit originRm dir of
      ...
      Just (linkTxt, destId, moveCost, maybeOriginMsg, maybeDestMsg) ->
          let ms'         = upd ms [ mobTbl.ind i.rmId      .~ destId
              ...
              originDesig = mkStdDesig i ms DoCap
              s           = getSing    i ms
          ...
          in (ms', Right (not (isSpiritId i ms) |?| bs, logMsg))
...
help (NoArgs i mq cols) = liftIO (T.readFile =<< mkMudFilePath rootHeplFileFun) |&| try >=> either handler helper
  where
    ...
    helper rootHelpTxt = ((,) <$> getState <*> getServerSettings) >>= \(ms, s) -> do
        ...
        let zipped                 = zip (styleAbbrevs Don'tCoins Don'tQuote [ helpName h | h <- hs ]) hs
  ...
  where
    helper ct ms =
        let (inInvs, inEqs, inRms) = sortArgsInvEqRm InRm as
            ...
            invCoins    = first (i `delete`) . getMobRmVisibleInvCoins i $ ms
            (eiss, ecs) = uncurry (resolveRmInvCoins i ms inRms) invCoins
            (pt, cbs,  logMsgs, intro'dIds) = foldl' (helperIntroEitherInv ct ms) (ms^.pcTbl, [], [], []) eiss
...
leave p@(WithArgs i mq cols (nub -> as)) =
  ...
  where
    ...
    mkMsgHelper isPlur (map dblQuote -> ns) =
        T.concat [ focusingInnateMsg, "you sever your telepathic connection", sOnTrue isPlur, " to the "
    ...
    flip maybeVoid res $ \(meLinkedToOthers, othersLinkedToMe, twoWays) ->
        let msgs                         = intercalate mMempty . dropEmpties $ [ twoWays       |!| twoWayMsgs
            ...
            mkSingsList doStyle ss = let (awakes, asleeps) = sortAwakesAsleeps ss
                                     in commas $ onTrue doStyle (styleAbbrevs Don'tCoins Don'tQuote) awakes ++ asleeps
          ...
          PlaType ->
            let (srcIntros, targetIntros) = f getIntroduced
                ...
                targetMsg | x <- "You sense an ephemeral blip in your psionic energy field as "
                          , y <- " establishes a telepathic connection from "
                          , z <- " mind to yours."
            ...
            mkBsForTarget targetDesig acc =
                let targetId = desigId targetDesig
                    ...
                    toOthers = ( nl . T.concat $ [ selfDesig', " looks at ", serialize targetDesig, "." ]
                               , targetId `delete` desigOtherIds selfDesig )
...
readyCloth i ms d mrol a@(et, _, _, _, _) clothId clothSing | em <- et IM.! i, cloth <- getCloth clothId ms =
  ...
  where
    mkReadyClothMsgs (pp -> slot) = \case
      Earring  -> wearMsgs
      NoseRing -> putOnMsgs d clothSing
      Necklace -> putOnMsgs d clothSing
      Bracelet -> wearMsgs
      Ring     -> slideMsgs
...
readyArm i ms d mrol a@(et, _, _, _, _) armId armSing | em <- et IM.! i, sub <- getArmSub armId ms =
  ...
  where
    ...
    mkReadyArmMsgs = ((d, armSing) |&|) . uncurry . \case
      Head   -> putOnMsgs
      Hands  -> putOnMsgs
      Feet   -> putOnMsgs
  ...
  where
      ...
      where
        ...
        sayToHelper d targetId targetDesig (frontAdv, rearAdv, msg) =
            let inLang           = mkInLangTxtForLang l
                ...
                toOthersBcasts   = [ (nl . mkToOthersMsg $ i', pure i') | i' <- targetId `delete` desigOtherIds d ]
                f                | isNpc i        ms = (, [])
    ...
    simpleSayHelper ms (maybeEmp spcL -> adverb) (formatMsg -> msg) =
        return $ let d                = mkStdDesig i ms DoCap
                     ...
                     toOthersBcasts   = [ (nl . mkToOthersMsg $ i', pure i') | i' <- desigOtherIds d ]
...
setAction (NoArgs i mq cols) = getState >>= \ms ->
    let (styleAbbrevs Don'tCoins Don'tQuote -> names, values) = unzip . mkSettingPairs i $ ms
    ...
    showInv ms d invCoins inInvs IdSingTypeDesig { .. }
      | ()!# invCoins =
          let (eiss, ecs)                         = uncurry (resolveMobInvCoins i ms inInvs) invCoins
              ...
              mkToSelfCoinsMsg  = coinTxt |!|                 T.concat   [ "You show ", coinTxt, " to ", theDesig, "." ]
              mkToTargetCoinsBs = coinTxt |!| mkBcast theId . T.concat $ [ serialize d, " shows you ", underline coinTxt, "." ]
              mkToOthersCoinsBs = coinTxt |!| [ ( T.concat [ serialize d, " shows ", aCoinSomeCoins canCoins, " to "
                                                           , theDesig, "." ]
                                                , theId `delete` desigOtherIds d ) ]
      ...
      | ()!# eqMap =
          let (gecrs, miss, rcs)                  = resolveEntCoinNames i ms inEqs (M.elems eqMap) mempty
              ...
              mkToSelfMsgs itemIds = [ T.concat [ "You show the ", getSing itemId ms, " to ", theDesig, "." ]
                                     | itemId <- itemIds ]
              mkToTargetBs itemIds = [ (T.concat [ serialize d, " shows you ", underline . aOrAn . getSing itemId $ ms
                                                 , let t = descSlotForId i ms itemId eqMap in t |!| spcL t, nl ":"
                                                 , getEntDesc itemId ms ], pure theId)
                                     | itemId <- itemIds ]
              mkToOthersBs itemIds = [ (T.concat [ serialize d, " shows ", aOrAn . getSing itemId $ ms
                                                 , let t = descSlotForId i ms itemId eqMap in t |!| spcL t, " to "
                                                 , theDesig, "." ], theId `delete` desigOtherIds d)
                                     | itemId <- itemIds ]
              -----
...
stanceAction (OneArgLower i mq cols a) = findFullNameForAbbrev a mkStancePairs |&| maybe notFound found
  where
    ...
    found (sta@(pp -> staTxt@(aOrAn -> staTxt')), _) = getState >>= \ms -> if getStance i ms == sta
      then ws . sorryStanceAlready $ staTxt
      else do logPla "stanceAction found" i . prd $ "setting stance to " <> staTxt
              tweak $ mobTbl.ind i.stance .~ sta
              ws $ "You take " <> staTxt' <> " combat stance."
              let msg = T.concat [ serialize . mkStdDesig i ms $ DoCap, " takes ", staTxt', " combat stance." ]
              when (isAttacking i ms) . bcastOthersInRm i $ msg
               ...
               found targetSing    =
                   let helper targetId = case emotifyTwoWay "telepathy" i ms targetId msg of
                       ...
                       mkStyled targetId = let (target'sAwakes, _) = getDblLinkedSings targetId ms
                                               styleds             = styleAbbrevs Don'tCoins Don'tQuote target'sAwakes
...
tune (NoArgs i mq cols) = getState >>= \ms ->
    let linkPairs   = map (dupFirst (`getIdForPCSing` ms)) . getLinked i $ ms
        ...
        styleds     = styleAbbrevs Don'tCoins Don'tQuote linkSings
    ...
    in do logPlaExecArgs "tune" [] i
          let msgs = [ helper "Two-way telepathic links:" styleds linkTunings
                     , mMempty
                     , helper "Telepathic channels:" (styleAbbrevs Don'tCoins Don'tQuote chanNames) chanTunings ]
