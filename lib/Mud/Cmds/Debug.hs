...
{-# LANGUAGE BangPatterns, ExistentialQuantification, LambdaCase, MonadComprehensions, NamedFieldPuns, OverloadedStrings, TypeApplications, ViewPatterns #-}
...
debugCmds =
    ...
    , mkDebugCmd "throw"       debugThrow       "Throw an exception (divide by zero)."
    , mkDebugCmd "throwlog"    debugThrowLog    "Throw an exception (divide by zero) on your player log thread."
...
descThreads = do logAsyncKvs <- getLogThreadIds >>= \case [ a, b ] -> return [ (a, Notice), (b, Error) ]
  ...
  where
    ...
    mkTypeName (LightTimer        pi) = f pi "LightTimer"
