...
createWorld = do
    ...
    putPla iRoot (Ent iRoot Nothing "Root" "" "This is the root admin." zeroBits) [] mempty M.empty (Mob Male 50 50 50 50 10 10 0 RHand) (PC iLoggedOut Human [] []) M.empty (Pla "" Nothing adminFlags 80 24 Nothing [] [] [] (Just iLounge))
    putPla iJason (Ent iJason Nothing "Jason" "" "Jason is the creator of CurryMUD." zeroBits) [] mempty M.empty (Mob Male 50 50 50 50 10 10 0 LHand) (PC iLoggedOut Human [] []) M.empty (Pla "" Nothing adminFlags 80 24 Nothing [] [] [] (Just iLounge))
